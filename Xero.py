...
HEADER = '\033\s*[^\n<]+"},
    ...
    # 8. Sensitive PII/Financial (Names, Address keywords, Phone formats)
    ...
    try:
        ...
        for key in EXTRACTION_PATTERNS.keys():
            ...
            if key == 1: # Logs/Errors (Full pattern match, excluding link text already added)
                 ...
                 # Find all matches for patterns that use capture groups
                 matches = re.findall(pattern, html_content, re.IGNORECASE | re.MULTILINE)

            # Add unique matches to the global tracker
            for match in matches:
                # If the regex uses a capture group, match might be a tuple/string, we ensure it's a string.
                if isinstance(match, tuple):
                    match = match if match else match[1] # Handle nested groups
                
                # Strip surrounding quotes and whitespace from captured values
