...
EXTRACTION_PATTERNS = {
    ...
    # 3. Username: Common fields near input tags (captures the value)
    ...
    # 5. Password: Common passwords/keys found in configurations (captures the value)
    ...
    # 7. Online Identifiers (IP addresses, UUIDs, etc.)
    7: {"name": "Online Identifiers (IPs/GUIDs)", "regex": r"\b(?:(?:\d{1,3}\.){3}\d{1,3})|([0-9a-fA-F]{8}-([0-9a-fA-F]{4}-){3}[0-9a-fA-F]{12})\b"},
    
    # 8. Sensitive PII/Financial (Names, Address keywords, Phone formats) (captures the value)
...
def extract_and_enqueue(html_content: str, base_url: str, file_handle: Optional = None):
    ...
    try:
        ...
        for tag in soup.find_all('a', href=True):
            ...
            # Acquisition of link text for Category 1 (Logs/Errors) - SYNTAX ERROR FIX
            if link_text and len(link_text) > 4:
                # Line 131 FIX: total_extracted_data.[1]add (Removed the '.')
            ...
            # Line 144 FIX: total_extracted_data.[2]add (Removed the '.')
        ...
        for key in EXTRACTION_PATTERNS.keys():
            ...
            for match in matches:
                if isinstance(match, tuple):
                    match = next((item for item in match if item), match)
                
                total_extracted_data[key].add(match.strip('\'" '))
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'text' in content_type or 'application/json' in content_type:
                    ...
                    # Add content to 'Logs/Errors' (Category 1) - SYNTAX ERROR FIX
                    # Line 183 FIX: total_extracted_data.[1]add (Removed the '.')
...
def main():
    ...
    try:
        ...
        if url_input!= "N/A":
            if file_handle is None or (file_handle and file_handle.tell() == 0):
                # Ensure a non-empty report is written if the crawl failed immediately
